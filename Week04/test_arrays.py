...
def test_names():
    ...
    """
    for f in files:
        assert "replace_center_with_minus_one" in dir(eval(f[:-3])), (
            "function is not defined in " + f[:-3]
        )
...
def test_callables():
    ...
    """
    for f in files:
        assert callable(eval(f[:-3]).replace_center_with_minus_one), (
            "function is not callable in " + f[:-3]
        )
...
def test_values_outside_center():
    ...
    for f in files:
        for d in range(1, 10):
            ...
            assert (
                outside_center.min() >= 0
            ), "Values outside the center should be non-negative"
            assert outside_center.min() >= 10 ** (
                d - 1
            ), f"Values outside the center should be more than {10 ** (d - 1)}"
            assert (
                outside_center.max() < 10**d
            ), f"Values outside the center should be less than {10 ** d}"
...
def test_center_for_odd_n_and_even_m():
    ...
    for f in files:
        ...
        assert np.all(
            center == -1
        ), "The center of the array should be all -1s for odd n and even m"
...
def test_center_for_even_n_and_odd_m():
    ...
    for f in files:
        ...
        assert np.all(
            center == -1
        ), "The center of the array should be all -1s for even n and odd m"
